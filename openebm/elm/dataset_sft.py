...
输出格式与 EBT pretrain dataloader 一致: (inputs, targets) 形状 (B, T)。
SFT targets 中非 assistant 监督位置会被置为 -1，以匹配 loss 的 ignore_index。
"""
...
class SFTIterableDataset(_IterableDataset):
    ...
    def _can_restore(self, state):
        return isinstance(state, dict) and state.get("state_version") == self.STATE_VERSION
    ...
    def __iter__(self):
        ...
        while True:
            ...
            for _ in range(self.B):
                ...
                while len(row) < self.row_capacity:
                    ...
                    if best_idx >= 0:
                        ...
                        row.extend(conv["ids"])
                        label_row.extend(conv["label_mask"])
                        self.consumed += ddp_world_size
                    else:
                        remaining = self.row_capacity - len(row)
                        row.extend([bos_token] * remaining)
                        label_row.extend([0] * remaining)
                        break

                rows.append(row[:self.row_capacity])
                label_rows.append(label_row[:self.row_capacity])

            batch_tensor = torch.tensor(rows, dtype=torch.long, pin_memory=use_cuda)
            label_mask_tensor = torch.tensor(label_rows, dtype=torch.bool, pin_memory=use_cuda)
            ...
            target_label_mask = label_mask_tensor[:, 1:].to(device=self.device, non_blocking=use_cuda)
            targets = torch.where(target_label_mask, targets, torch.full_like(targets, -1))
