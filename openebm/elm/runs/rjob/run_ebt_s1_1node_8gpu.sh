...
################################################################################
# EBT d26 S1 profiling training script - 1节点 × 8卡
#
# 用法:
#   通过 rjob 提交时，将任务命令指向本脚本
#   本地调试: NODE_RANK=0 NODE_COUNT=1 MASTER_ADDR=127.0.0.1 PROC_PER_NODE=8 bash run_ebt_s1_1node_8gpu.sh
#
################################################################################
...
### 基础配置 ###
RUN_PREFIX="s1-1node-8gpu-profile"
...
PROFILE_TRAINING_PIPELINE=${PROFILE_TRAINING_PIPELINE:-true}
PROFILE_FLAGS=""
if [[ "${PROFILE_TRAINING_PIPELINE}" == "true" ]]; then
    PROFILE_FLAGS="--profile_training_pipeline --log_every_n_steps 1"
fi
...
ENABLE_NVIDIA_MONITOR=${ENABLE_NVIDIA_MONITOR:-true}
NVIDIA_SMI_INTERVAL=${NVIDIA_SMI_INTERVAL:-5}
ENABLE_NVIDIA_DMON=${ENABLE_NVIDIA_DMON:-false}
...
GPU_MONITOR_DIR="${LOG_DIR}/gpu_monitor"
mkdir -p "${GPU_MONITOR_DIR}"
NVIDIA_SMI_LOG_FILE="${GPU_MONITOR_DIR}/${RUN_NAME}_rank${NODE_RANK}_nvidia_smi.csv"
NVIDIA_DMON_LOG_FILE="${GPU_MONITOR_DIR}/${RUN_NAME}_rank${NODE_RANK}_nvidia_dmon.log"

NVIDIA_SMI_MONITOR_PID=""
NVIDIA_DMON_MONITOR_PID=""

start_nvidia_monitor() {
    if [[ "${ENABLE_NVIDIA_MONITOR}" != "true" ]]; then
        echo "NVIDIA monitor disabled (ENABLE_NVIDIA_MONITOR=${ENABLE_NVIDIA_MONITOR})"
        return
    fi

    if ! command -v nvidia-smi >/dev/null 2>&1; then
        echo "WARNING: nvidia-smi not found; skip NVIDIA monitor"
        return
    fi

    echo "Starting nvidia-smi monitor:"
    echo "  interval: ${NVIDIA_SMI_INTERVAL}s"
    echo "  output:   ${NVIDIA_SMI_LOG_FILE}"
    nvidia-smi \
      --query-gpu=timestamp,index,uuid,name,utilization.gpu,utilization.memory,memory.used,memory.total,power.draw,clocks.sm,clocks.mem,temperature.gpu,pcie.link.gen.current,pcie.link.width.current \
      --format=csv \
      -l "${NVIDIA_SMI_INTERVAL}" \
      > "${NVIDIA_SMI_LOG_FILE}" 2>&1 &
    NVIDIA_SMI_MONITOR_PID=$!
    echo "  pid:      ${NVIDIA_SMI_MONITOR_PID}"

    if [[ "${ENABLE_NVIDIA_DMON}" == "true" ]]; then
        echo "Starting nvidia-smi dmon monitor:"
        echo "  output:   ${NVIDIA_DMON_LOG_FILE}"
        nvidia-smi dmon -s pucm -d "${NVIDIA_SMI_INTERVAL}" \
          > "${NVIDIA_DMON_LOG_FILE}" 2>&1 &
        NVIDIA_DMON_MONITOR_PID=$!
        echo "  pid:      ${NVIDIA_DMON_MONITOR_PID}"
    fi
}

stop_nvidia_monitor() {
    local pid
    for pid in "${NVIDIA_SMI_MONITOR_PID}" "${NVIDIA_DMON_MONITOR_PID}"; do
...
echo "PROFILE_TRAINING_PIPELINE: ${PROFILE_TRAINING_PIPELINE}"
echo "ENABLE_NVIDIA_MONITOR: ${ENABLE_NVIDIA_MONITOR}"
echo "NVIDIA_SMI_INTERVAL: ${NVIDIA_SMI_INTERVAL}"
echo "NVIDIA_SMI_LOG_FILE: ${NVIDIA_SMI_LOG_FILE}"

exec > >(tee -a "${LOG_FILE}") 2>&1

set +e

trap stop_nvidia_monitor EXIT INT TERM
start_nvidia_monitor
...
echo "日志文件: ${LOG_FILE}"
echo "NVIDIA SMI 监控文件: ${NVIDIA_SMI_LOG_FILE}"
if [[ "${ENABLE_NVIDIA_DMON}" == "true" ]]; then
    echo "NVIDIA dmon 监控文件: ${NVIDIA_DMON_LOG_FILE}"
fi
