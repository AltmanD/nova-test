...
################################################################################
# EBT d26 S2 fixed-alpha 训练脚本 - 2节点 × 8卡
#
# 设计目标:
#   在 onlytruncate 的平滑 BPB / 稳定 CE 路径上固定 alpha=397。
#   本脚本不启用 SIGReg，也暂不打开 no_mcmc_detach、Langevin、
#   randomized repeated steps，避免一次性引入多个会抬高或抖动 valid_bpb 的变量。
#   适合作为 fixed-alpha 无 SIGReg 对照组。
#
# 用法:
#   通过 rjob 提交时，将提交脚本中的 RUN_SCRIPT 指向本文件
#   本地调试: NODE_RANK=0 NODE_COUNT=1 MASTER_ADDR=127.0.0.1 PROC_PER_NODE=8 bash run_ebt_s2_fixedalpha397_2node_8gpu.sh
#
################################################################################
...
### 基础配置 ###
RUN_PREFIX="s2-fixedalpha397-2node-8gpu-bf16mixed"
...
# EBT 核心超参数
MCMC_STEP_SIZE=397.0                 # 固定 alpha，不参与学习
...
# 保留 onlytruncate 的稳定项:
#   - 2 个 MCMC landscape
#   - truncate_mcmc，仅用最终步 CE
#   - fixed alpha=397，避免 alpha 学习过程掩盖 S2 变量影响
#
# 暂不启用 syncebt 中更激进的 S2 项:
#   - --no_mcmc_detach
#   - randomized repeated MCMC steps
#   - Langevin dynamics
#   - randomized alpha scale
#
# 这样可以作为 fixed-alpha 无 SIGReg baseline，用于和 fixed-alpha + SIGReg 对照。
