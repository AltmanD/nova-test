...
################################################################################
# EBT d26 S2-SIGReg SFT training script (offline mode)
#
# Fine-tunes a base model trained by:
#   openebm/elm/runs/rjob/run_ebt_s2_sigreg_2node_8gpu.sh
#
# Usage:
#   PRETRAIN_CKPT=/path/to/base.ckpt bash openebm/elm/runs/sft/run_ebt_s2_sft_0520.sh
################################################################################
...
export EBT_RUNS_ROOT="${EBT_RUNS_ROOT:-${NOVA_HOME}/logs/ebt_runs}"

################################################################################
# Basic config
################################################################################

# EXP_ID points to the corresponding base_train experiment family.
export EXP_ID="${EXP_ID:-d26-ctx2048-20260520-s2-sigreg}"
...
# Keep this explicit to avoid accidentally SFT-ing from a non-SIGReg checkpoint.
PRETRAIN_CKPT="/mnt/shared-storage-user/luyudong/nova/logs/checkpoints/s2-sigreg-2node-8gpu-bf16mixed_0520_1403_d26_ctx2048_bs512_lr0.0012_2nodes_8gpus/s=step=6999-d26-ctx2048-lr0.0012-bs1x32-muon_adamw-valid_loss=valid_loss=2.5764.ckpt"
...
################################################################################
# EBT core hyperparameters aligned with run_ebt_s2_sigreg_2node_8gpu.sh
################################################################################

MCMC_STEP_SIZE=470.0
MCMC_NUM_STEPS=2
EBT_TYPE="time_embed"
...
################################################################################
# SIGReg config aligned with run_ebt_s2_sigreg_2node_8gpu.sh
################################################################################

SIGREG_LAMBDA=${SIGREG_LAMBDA:-0.01}
SIGREG_NUM_SLICES=${SIGREG_NUM_SLICES:-512}
SIGREG_NUM_POINTS=${SIGREG_NUM_POINTS:-17}
SIGREG_WARMUP_STEPS=${SIGREG_WARMUP_STEPS:-1000}

SIGREG_FLAGS="--sigreg_lambda ${SIGREG_LAMBDA} --sigreg_num_slices ${SIGREG_NUM_SLICES} --sigreg_num_points ${SIGREG_NUM_POINTS} --sigreg_warmup_steps ${SIGREG_WARMUP_STEPS}"
...
# Base LR is 0.0012 in run_ebt_s2_sigreg_2node_8gpu.sh; SFT uses a 5x lower main LR.
PEAK_LR=0.00024
...
if [ -z "${PRETRAIN_CKPT}" ]; then
    echo "PRETRAIN_CKPT is required."
    echo "Usage:"
    echo "  PRETRAIN_CKPT=/path/to/base.ckpt bash $0"
    exit 1
fi
...
exp_init_sft "$0"
EXP_STAGE_DIR="$(dirname "$(dirname "${EXP_LOG_FILE}")")"
export RUN_NAME="${EXP_ID}-sft"
export EXP_START_TIME="$(date '+%Y-%m-%d %H:%M:%S')"

exp_save_hparams "${EXP_STAGE_DIR}" \
    "model_size=${MODEL_SIZE}" \
    "context_length=${CONTEXT_LENGTH}" \
    "peak_lr=${PEAK_LR}" \
    "sft_muon_lr=${SFT_MUON_LR}" \
    "sft_embedding_lr=${SFT_EMBEDDING_LR}" \
    "sft_vocab_to_embed_lr=${SFT_VOCAB_TO_EMBED_LR}" \
    "sft_scalar_lr=${SFT_SCALAR_LR}" \
...
echo "SIGReg flags:  ${SIGREG_FLAGS}"
...
################################################################################
#                    EBT d26 S2-SIGReg SFT training log
################################################################################
#
...
echo "No MCMC Detach:      ${NO_MCMC_DETACH}"
echo "SIGReg Flags:        ${SIGREG_FLAGS}"
echo ""
echo "================================================================================"
echo "[START TRAINING]"
...
exp_save_status "${EXP_STAGE_DIR}" "$(basename "${EXP_STAGE_DIR}")" "${TRAIN_EXIT_CODE}"
