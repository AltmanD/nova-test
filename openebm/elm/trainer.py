...
class ModelTrainer(LightningModule):
    ...
    def _reset_profile_window(self):
        ...
        self._profile_window = {
            ...
            "optimizer_s_sum": 0.0,
            "data_wait_s_max": 0.0,
            "dl_sums": {key: 0.0 for key in dl_sum_keys},
            "dl_counts": {key: 0 for key in dl_sum_keys},
            "dl_maxes": {key: 0.0 for key in dl_max_keys},
            "dl_mins": {"remaining_tokens_min": float("inf")},
            "dl_last": {},
        }
        ...
        if dl:
            ...
            for key in window["dl_sums"].keys():
                ...
                if finite_number(value):
                    window["dl_sums"][key] += float(value)
                    window["dl_counts"][key] += 1
            for key in window["dl_maxes"].keys():
                value = dl.get(key)
                if finite_number(value):
                    window["dl_maxes"][key] = max(window["dl_maxes"][key], float(value))
            for key in window["dl_mins"].keys():
                value = dl.get(key)
                if finite_number(value):
                    window["dl_mins"][key] = min(window["dl_mins"][key], float(value))
            window["dl_last"] = dl
    ...
    def _emit_profile_step_summary(self):
        ...
        def avg_profile(key):
            count = int(window["dl_counts"].get(key, 0))
            return dl_sums.get(key, 0.0) / count if count > 0 else float("nan")

        def sum_profile(key):
            return dl_sums.get(key, 0.0)

        def max_profile(key):
            return window["dl_maxes"].get(key, float("nan"))

        def min_profile(key):
            value = window["dl_mins"].get(key, float("nan"))
        ...
        packed_tokens = sum_profile("packed_tokens")
        cropped_tokens = sum_profile("cropped_tokens")
        total_tokens = packed_tokens + cropped_tokens
        crop_ratio = cropped_tokens / total_tokens if total_tokens > 0 else float("nan")
        ...
        print(
            ...
            f"dl_avg_ms={avg_profile('batch_total_ms'):.2f} "
            f"dl_max_ms={max_profile('batch_total_ms'):.2f} "
            f"fetch_avg_ms={avg_profile('refill_fetch_ms'):.2f} "
            f"tok_avg_ms={avg_profile('tokenize_ms'):.2f} "
            f"tok_max_ms={max_profile('tokenize_ms'):.2f} "
            f"pack_avg_ms={avg_profile('packing_scan_ms'):.2f} "
            f"pack_max_ms={max_profile('packing_scan_ms'):.2f} "
            f"doc_select_avg_ms={avg_profile('doc_select_ms'):.2f} "
            f"doc_select_max_ms={max_profile('doc_select_ms'):.2f} "
            f"crop_branch_avg_ms={avg_profile('crop_branch_ms'):.2f} "
            f"crop_branch_max_ms={max_profile('crop_branch_ms'):.2f} "
            f"tensor_avg_ms={avg_profile('tensor_materialize_ms'):.2f} "
            f"tensor_max_ms={max_profile('tensor_materialize_ms'):.2f} "
            f"row_write_avg_ms={avg_profile('row_buffer_write_ms'):.2f} "
            f"row_write_max_ms={max_profile('row_buffer_write_ms'):.2f} "
            f"cpu_batch_copy_avg_ms={avg_profile('cpu_batch_copy_ms'):.2f} "
            f"cpu_batch_copy_max_ms={max_profile('cpu_batch_copy_ms'):.2f} "
            f"state_dict_avg_ms={avg_profile('state_dict_ms'):.2f} "
            f"state_dict_max_ms={max_profile('state_dict_ms'):.2f} "
            f"unaccounted_avg_ms={avg_profile('unaccounted_ms'):.2f} "
            f"unaccounted_max_ms={max_profile('unaccounted_ms'):.2f} "
            f"copy_avg_ms={avg_profile('cpu_to_gpu_copy_ms'):.2f} "
            f"h2d_copy_avg_ms={avg_profile('h2d_copy_ms'):.2f}",
            flush=True,
        )
        print(
            ...
            f"docbuf_start_avg={avg_profile('doc_buffer_len_start'):.2f} "
            f"docbuf_end={int(dl_last.get('doc_buffer_len_end', -1))} "
            f"docbuf_max={as_int(max_profile('doc_buffer_len_max'))} "
            f"refill_calls={as_int(sum_profile('refill_calls'), 0)} "
            f"selected_docs={as_int(sum_profile('selected_docs'), 0)} "
            f"packed_docs={as_int(sum_profile('packed_docs'), 0)} "
            f"cropped_docs={as_int(sum_profile('cropped_docs'), 0)} "
            f"packed_tok={as_int(packed_tokens, 0)} "
            f"cropped_tok={as_int(cropped_tokens, 0)} "
            f"crop_ratio={crop_ratio:.4f} "
            f"doc_len_mean={avg_profile('doc_len_mean'):.2f} "
            f"doc_len_p50={avg_profile('doc_len_p50'):.2f} "
            f"doc_len_p90={avg_profile('doc_len_p90'):.2f} "
            f"doc_len_p99={avg_profile('doc_len_p99'):.2f} "
            f"doc_len_max={max_profile('doc_len_max'):.2f} "
            f"remaining_mean={avg_profile('remaining_tokens_mean'):.2f} "
            f"remaining_min={min_profile('remaining_tokens_min'):.2f} "
            f"remaining_max={max_profile('remaining_tokens_max'):.2f}",
            flush=True,
        )
    ...
    def log_metrics(self, metrics_dict, phase, log_torchmetrics = True):
        ...
        if phase == "train" and hasattr(self, 'trainer') and self.trainer is not None:
            ...
            if self.trainer.is_global_zero:
                ...
                # Profiling mode emits an optimizer-step aggregated tok/sec summary.
                num_gpus = getattr(self.hparams, 'num_gpus', 1)
